...
import { FaCircle, FaLeaf, FaSpinner } from "react-icons/fa";
...
const GreenMutualFund = () => {
    ...
    const fetchFunds = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/api/green-funds"
        );
  ...
  const handleFundSelect = async (fund) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5000/api/recommendations/${fund.id}`
      );
  ...
  const filteredFunds = funds.filter((fund) => {
    ...
    return (
      category &&
      fund.esgScore >= category.range[0] &&
      fund.esgScore <= category.range[1]
    );
  });
              ...
              filteredFunds.map((fund) => (
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div
                      className={`text-lg font-bold ${getESGScoreColor(
                        fund.esgScore
                      )}`}
                    >
                  ...
                  <div className="grid grid-cols-2 gap-4 bg-gray-700 p-4 rounded-lg">
                    ...
                    <div>
                      ...
                      <div className="flex">
                        {[...Array(fund.sustainabilityRating || 0)].map(
                          (_, i) => (
